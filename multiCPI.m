...
%% Time Vector %%%
t = 0:Ts:(CPI*Np*Tp-Ts); % time vector
...
%% Combines Signal and Noise
za = za1+za2; %Receiver receives both the up and down ramps
...
%% Match Filter For Each CPI
for iCPI = 1:length(CPI)
    ...
    range_d1(iCPI) = taugrid(d1)*c/2;
    range_d2(iCPI) = taugrid(d2)*c/2;

    % Find Velocity
    [~,s1] = max(abs(rangedopplerUp(:,d1)));
    [~,s2] = max(abs(rangedopplerDown(:,d2)));

    radial_speed1(iCPI) = nugrid(s1)*fp*c/(2*fc) %converts to real frequency then to velocity
    radial_speed2(iCPI) = nugrid(s2)*fp*c/(2*fc); %converts to real frequency then to velocity
